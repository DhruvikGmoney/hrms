...
module.exports = {
    ...
    login: async (req, res) => {
        ...
        if (user.email == email && (bcrypt.compare(password, user.password))) {
            let token = jsonwebtoken.sign({ id: user._id, email: email, role: user.role }, process.env.JWT_SECRET_KEY, { expiresIn: "10h" })
            return res.status(200).json({ email, token })
        } else {
            return res.status(401).json({ message: "Please Provide Valid Email And Password" })
        }
    },
